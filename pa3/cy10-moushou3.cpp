/*
 ...
 1:魔兽世界三(开战)
 write by xucaimao,2018-11-07,AC at
*/
...
class Weapon{
private:
    Warrior *pWarrior;  //所属士兵
    int weaponId;
    int force;          //武器的攻击力
public:

    static char weaponName[3][6];   //三种武器sword,bomb,和arrow，编号分别为0,1,2。
};
...
private:
    ...
    int weapon[2];      //用于存放武器的型号,武器有三种
    double morale;      //士气值
    int loyal;          //忠诚度
    int loyalDec;       //忠诚度降低，只有lion有次属性，其他设置为0
    Weapon *pWeapon[10];//士兵武器总数不超过十件
...
public:
    ...
    static int warriorForce[5];     //士兵的攻击力
...
Warrior::Warrior(Headquarter * p,int life_,int kindNo_,int No_,int color_,int ld_=0):pHead{p}{
    life=life_;
    kindNo=kindNo_;
    No=No_;
    color=color_;
    ...
    loyalDec=ld_;
    morale=0;
    force=warriorForce[kindNo];     //设置初始攻击力
    //下面的武器部分要修改
    switch(kindNo){
        case 0: //dragon
            weaponNum=1;
...
int Warrior::warriorForce[5];
char Weapon::weaponName[3][6]={"sword","bomb","arrow"};
...
int main(){
    ...
    for(int i=1;i<=caseNum;i++){//处理各组数据
        int curTime=0;
        cin>>M>>N>>K>>T;
        for(int j=0;j<5;j++)//读入初始生命值
            cin>>Headquarter::valueOfMake[j];
        for(int j=0;j<5;j++)//读入攻击力
            cin>>Warrior::warriorForce[j];
